...
plugin.description = "Monaco Editor 替换原始 textarea，浏览时只读显示，编辑时可修改，支持自动语言识别、未知文件手动选择高亮、保存前 Diff 对比、移动端触摸选中与全选。";
plugin.tags = ["编辑器", "Monaco", "语法高亮", "自动识别", "手动高亮", "Diff", "只读浏览", "移动端", "中文"];
...
/* 移动端触摸拖动选择时禁用滚动惯性干扰 */
.monaco-enhance-container.dragging-select .monaco-editor,
.monaco-enhance-container.dragging-select .monaco-editor .view-lines {
...
// ===== Monaco 中文语言包 =====
// 通过设置 require 的 "vs/nls" available languages 实现界面中文化
const MONACO_NLS_CN = {
  // 仅声明使用 zh-cn，实际文案由 CDN 的 nls 包提供
};
        ...
        getWorkerUrl: function () {
          return `data:text/javascript;charset=utf-8,${encodeURIComponent(`
            self.MonacoEnvironment = {
              baseUrl: '${MONACO_CDN}/',
              getWorkerUrl: function(){ return '${MONACO_CDN}/base/worker/workerMain.js'; }
            };
...
// ===== 移动端工具条：拖动选中 + 全选 + 复制 =====
// ============================================================
function createMobileToolbar(monaco, editor, container) {
  const bar = document.createElement("div");
  bar.className = "monaco-enhance-mobilebar";

  // —— 拖动选中按钮（开关）——
  const selectBtn = document.createElement("button");
  selectBtn.className = "monaco-enhance-mbtn";
  selectBtn.textContent = "✋ 拖动选中";

  // —— 全选按钮 ——
  const allBtn = document.createElement("button");
  allBtn.className = "monaco-enhance-mbtn";
  allBtn.textContent = "全选";

  // —— 复制按钮 ——
  const copyBtn = document.createElement("button");
  copyBtn.className = "monaco-enhance-mbtn";
  ...
  // ===== 拖动选中（触摸）=====
  // 思路：开启「拖动选中」模式后，监听编辑器 DOM 的 touch 事件，
  // 把触摸坐标转换为 Monaco 的位置（行/列），手动设置选区。
  let dragMode = false;
  let anchorPos = null;
  ...
  const onTouchStart = (e) => {
    ...
    const t = e.touches[0];
    if (!t) return;
    const pos = posFromTouch(t);
    if (!pos) return;
    anchorPos = pos;
    editor.setPosition(pos);
    container.classList.add("dragging-select");
    e.preventDefault();
    e.stopPropagation();
  };

  const onTouchMove = (e) => {
    if (!dragMode || !anchorPos) return;
    const t = e.touches[0];
    if (!t) return;
    const pos = posFromTouch(t);
    if (!pos) return;
  ...
  const onTouchEnd = (e) => {
    if (!dragMode) return;
    anchorPos = null;
    container.classList.remove("dragging-select");
  };

  const bindTouch = () => {
    if (!editorDom) return;
    editorDom.addEventListener("touchstart", onTouchStart, { passive: false });
    editorDom.addEventListener("touchmove", onTouchMove, { passive: false });
    editorDom.addEventListener("touchend", onTouchEnd, { passive: false });
    editorDom.addEventListener("touchcancel", onTouchEnd, { passive: false });
  };
  ...
  const unbindTouch = () => {
    ...
    editorDom.removeEventListener("touchcancel", onTouchEnd);
  };
...
let activeMobileCleanup = null;

function disposeActive() {
  if (activeMobileCleanup) {
    try { activeMobileCleanup(); } catch (e) {}
    activeMobileCleanup = null;
  }
...
function showDiffDialog(monaco, oldText, newText, fileName) {
  return new Promise((resolve) => {
    ...
    head.innerHTML = `
      <div class="mdiff-title">提交确认 · 差异对比</div>
    ...
    // 移动端用上下对比（行宽不够时更友好）
    const sideBySide = !isMobileDevice();
...
plugin.onHook = async (hookName, data) => {
  ...
  if (hookName === "file:open") {
    ...
    const editor = monaco.editor.create(container, {
      ...
      // 移动端体验优化
      scrollbar: {
        verticalScrollbarSize: mobile ? 10 : 12,
        horizontalScrollbarSize: mobile ? 10 : 12,
      ...
      // 中文无障碍提示
      ariaLabel: "代码编辑器，支持只读浏览、编辑、全选、复制和拖动选中"
    });
    ...
    // 移动端创建底部工具条：拖动选中 / 全选 / 复制
    if (mobile) {
      activeMobileCleanup = createMobileToolbar(monaco, editor, container);
    }
      ...
      saveBtn.addEventListener(
        ...
        async function diffGate(e) {
          ...
          try {
            ...
            alert("Monaco 加载失败，无法显示 Diff。");
            return;
          }

          const confirmed = await showDiffDialog(
            mon,
            originalText,
            newText,
            currentFile.name
