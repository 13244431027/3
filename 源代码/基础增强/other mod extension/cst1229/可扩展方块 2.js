...
// 描述: 如果 Extendable Blocks 这么好，为什么没有 Extendable Blocks 2？
...
(function (Scratch) {
	...
	if (!Scratch.extensions.unsandboxed) {
		throw new Error("可扩展方块 2 必须在非沙盒模式下运行");
	}
	...
	for (let i = MIN_INPUTS; i <= MAX_INPUTS; i++) {
		...
		blocks.push(...([
			...
			sep(),
			{
				...
				blockType: Scratch.BlockType.REPORTER,
				text: "连接 " + inputsFor("ARG").join("") + " 和 [ARG" + inputCount + "]",
				arguments: {
			...
			},
			{
				...
				blockType: Scratch.BlockType.REPORTER,
				text: "创建 JSON 数组 " + inputsFor("ARG").join(""),
				arguments: argsFor("ARG", Scratch.ArgumentType.STRING, ""),
				extensions: ["colours_operators"],
				disableMonitor: true,
			},
			...
			},
			{
				...
				text: branches(inputCount + 1, (i) => {
					if (i === 0) {
						return `如果 [CONDITION${i}] 则`;
					}
					return `否则如果 [CONDITION${i}] 则`;
				}),
	...
	class ExtendableBlocks2 {
		...
		getInfo() {
			return {
				...
				blocks: [
					// 我本来打算在调色板里加按钮来增删输入，但把所有方块一次性显示更有趣
					/*
					{
