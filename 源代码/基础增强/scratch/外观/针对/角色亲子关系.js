// 名称: 角色亲子关系
// ID: SPspriteParent
// 描述: 将角色链接在一起，使它们跟随父角色。
// 作者: SharkPool

// 版本 2.0.0

(function (Scratch) {
  "use strict";
  if (!Scratch.extensions.unsandboxed) throw new Error("角色亲子关系必须在非沙盒环境中运行");

  const vm = Scratch.vm;
  const runtime = vm.runtime;

  const menuIconURI =
"data:image/svg+xml;base64,";

  // 感谢 DogeIsCut 提供了菜单图标的灵感并允许我使用他的想法制作扩展 :3
  const blockIconURI =
"data:image/svg+xml;base64,";

  class SPspriteParent {
    constructor() {
      this.links = Object.create(null);
    }

    getInfo() {
      return {
        id: "SPspriteParent",
        name: "角色亲子关系",
        color1: "#4B21CC",
        menuIconURI,
        blockIconURI,
        blocks: [
          { blockType: Scratch.BlockType.LABEL, text: "链接管理" },
          {
            opcode: "makeLink",
            blockType: Scratch.BlockType.COMMAND,
            text: "创建新角色组 [NAME]",
            arguments: {
              NAME: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "组1"
              },
            },
          },
          {
            opcode: "deleteLink",
            blockType: Scratch.BlockType.COMMAND,
            text: "删除角色组 [NAME]",
            arguments: {
              NAME: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "组1"
              },
            },
          },
          {
            opcode: "deleteAll",
            blockType: Scratch.BlockType.COMMAND,
            text: "删除所有角色组",
          },
          "---",
          {
            opcode: "addSprite",
            blockType: Scratch.BlockType.COMMAND,
            text: "将 [NAME] 加入角色组 [LINK]",
            arguments: {
              LINK: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "组1"
              },
              NAME: {
                type: Scratch.ArgumentType.STRING,
                menu: "TARGETS"
              }
            },
          },
          {
            opcode: "removeSprite",
            blockType: Scratch.BlockType.COMMAND,
            text: "将 [NAME] 移出角色组 [LINK]",
            arguments: {
              LINK: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "组1"
              },
              NAME: {
                type: Scratch.ArgumentType.STRING,
                menu: "TARGETS"
              }
            },
          },
          "---",
          {
            opcode: "linkExists",
            blockType: Scratch.BlockType.BOOLEAN,
            text: "角色组 [NAME] 存在？",
            arguments: {
              NAME: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "组1"
              }
            },
          },
          {
            opcode: "contentsLink",
            blockType: Scratch.BlockType.REPORTER,
            text: "角色组 [NAME] 的成员",
            arguments: {
              NAME: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "组1"
              }
            },
          },
          { blockType: Scratch.BlockType.LABEL, text: "父角色设置" },
          {
            opcode: "linkMaster",
            blockType: Scratch.BlockType.COMMAND,
            text: "设置 [NAME] 为角色组 [LINK] 的父角色",
            arguments: {
              NAME: {
                type: Scratch.ArgumentType.STRING,
                menu: "TARGETS"
              },
              LINK: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "组1"
              }
            },
          },
          {
            opcode: "whoMaster",
            blockType: Scratch.BlockType.REPORTER,
            text: "角色组 [LINK] 的父角色",
            arguments: {
              LINK: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "组1"
              }
            },
          },
          { blockType: Scratch.BlockType.LABEL, text: "跟随控制" },
          {
            opcode: "updateLink",
            blockType: Scratch.BlockType.COMMAND,
            text: "更新角色组 [LINK] 的 [TYPE]",
            arguments: {
              LINK: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "组1"
              },
              TYPE: {
                type: Scratch.ArgumentType.STRING,
                menu: "CONTROLTYPE"
              }
            },
          },
          {
            opcode: "updateAll",
            blockType: Scratch.BlockType.COMMAND,
            text: "更新所有角色组的 [TYPE]",
            arguments: {
              TYPE: {
                type: Scratch.ArgumentType.STRING,
                menu: "CONTROLTYPE"
              }
            },
          },
          "---",
          {
            opcode: "refreshLink",
            blockType: Scratch.BlockType.COMMAND,
            text: "刷新角色组 [LINK] 的位置",
            arguments: {
              LINK: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "组1"
              }
            },
          },
          {
            opcode: "refreshAll",
            blockType: Scratch.BlockType.COMMAND,
            text: "刷新所有角色组的位置"
          }
        ],
        menus: {
          TARGETS: {
            acceptReporters: true,
            items: "_getTargets",
          },
          CONTROLTYPE: {
            acceptReporters: true,
            items: [
              "全部属性", "坐标位置", "方向",
              "大小", "视觉效果", "显示隐藏"
            ],
          },
        },
      };
    }

    makeLink(args) { this.links[args.NAME] = {} }

    deleteLink(args) { delete this.links[args.NAME] }

    deleteAll(args) { this.links = Object.create(null) }

    addSprite(args, util) {
      const link = this.links[args.LINK];
      if (!link) this.links[args.LINK] = {};
      const target = args.NAME === "_myself_" ? util.target : runtime.getSpriteTargetByName(args.NAME);
      if (target) this.links[args.LINK][target.id] = [target, target.x, target.y];
    }

    removeSprite(args, util) {
      const link = this.links[args.LINK];
      if (link) {
        const target = args.NAME === "_myself_" ? util.target : runtime.getSpriteTargetByName(args.NAME);
        if (target) delete this.links[args.LINK][target.id];
      }
    }

    linkExists(args) { return Scratch.Cast.toBoolean(this.links[args.NAME]) }

    contentsLink(args) {
      const link = this.links[args.NAME];
      if (link !== undefined) {
        const spriteNames = Object.keys(link).filter(key => key !== "SPlinkMaster")
          .map(key => {
            const sprite = link[key][0];
            const name = sprite.getName();
            if (!sprite.isOriginal) return name + " (副本)";
            else return name;
        });
        return JSON.stringify(spriteNames);
      } else { return "该角色组不存在" }
    }

    linkMaster(args, util) {
      const link = this.links[args.LINK];
      if (!link) this.links[args.LINK] = {};
      const target = args.NAME === "_myself_" ? util.target : runtime.getSpriteTargetByName(args.NAME);
      if (target) {
        this.links[args.LINK].SPlinkMaster = {
          sprite: target.getName(),
          id: target.id,
          direction: target.direction,
          size: target.size,
          show: target.visible,
          effects: target.effects
        };
      }
    }

    updateMaster() {
      for (const link in this.links) {
        const newLink = this.links[link];
        if (newLink.SPlinkMaster !== undefined) {
          const target = runtime.getTargetById(newLink.SPlinkMaster.id);
          newLink.SPlinkMaster = {
            sprite: target.getName(),
            id: target.id,
            direction: target.direction,
            size: target.size,
            show: target.visible,
            effects: target.effects
          };
        }
      }
    }

    whoMaster(args) {
      if (this.links[args.LINK] !== undefined) {
        const master = this.links[args.LINK].SPlinkMaster;
        if (master !== undefined) return master.sprite;
      }
      return "该角色组没有父角色";
    }

    refresh(LINK) {
      this.updateMaster();
      if (this.links[LINK]) {
        let link = this.links[LINK];
        Object.keys(link).filter(key => key !== "SPlinkMaster")
          .map(key => {
            // 获取角色的新位置信息
            const target = runtime.getTargetById(key);
            if (target) link[key] = [target, target.x, target.y];
        });
      }
    }

    refreshLink(args) { this.refresh(args.LINK) }

    refreshAll() {
      const linkKeys = Object.keys(this.links);
      for (let i = 0; i < linkKeys.length; i++) {
        this.refresh(linkKeys[i]);
      }
    }

    updateLink(args) { this.update(args.LINK, args.TYPE) }

    updateAll(args) {
      const linkKeys = Object.keys(this.links);
      for (let i = 0; i < linkKeys.length; i++) {
        this.update(linkKeys[i], args.TYPE);
      }
    }

    update(link, TYPE) {
      this.updateMaster();
      link = this.links[link];
      if (link) {
        const master = link.SPlinkMaster;
        const sprites = Object.keys(link).filter(key => key !== "SPlinkMaster");
        if (!master) throw new Error("该角色组没有设置父角色");
        const masTarget = runtime.getTargetById(master.id);
        for (let i = 0; i < sprites.length; i++) {
          const target = link[sprites[i]][0];
          if (target) {
            if (TYPE === "全部属性" || TYPE === "方向") {
              target.setDirection(master.direction);
            }
            if (TYPE === "全部属性" || TYPE === "大小") {
              target.setSize(master.size);
            }
            if (TYPE === "全部属性" || TYPE === "显示隐藏") {
              target.setVisible(master.show);
            }
            if (TYPE === "全部属性" || TYPE === "视觉效果") {
              let value = master.effects;
              for (const effect in value) {
                target.setEffect(effect, value[effect]);
              }
            }
            if (TYPE === "全部属性" || TYPE === "坐标位置") {
              target.setXY(link[sprites[i]][1] + masTarget.x, link[sprites[i]][2] + masTarget.y);
            }
          }
        }
      }
    }

    // 感谢 LilyMakesThings 提供的这段宝贵代码 :3
    _getTargets() {
      const spriteNames = [];
      spriteNames.push({ text : "当前角色", value: "_myself_" });
      const targets = Scratch.vm.runtime.targets;
      for (let index = 1; index < targets.length; index++) {
        const target = targets[index];
        if (target.isOriginal) spriteNames.push({ text : target.getName(), value : target.getName() });
      }
      return spriteNames.length > 0 ? spriteNames : [""];
    }
  }

  Scratch.extensions.register(new SPspriteParent());
})(Scratch);