...
class UIComponents {
  ...
  static createAIPopup() {
    ...
    body.textContent = '你好！我是你的 GitHub 助手。';
    ...
    return { popup: pop, input: inp, body: body, close: close };
  }
}
...
class PluginManager {
  constructor(extension) {
    ...
    this.hooks = new Map();
    setTimeout(() => this._loadFromStorage(), 100);
  }
  ...
  _loadFromStorage() {
    try {
      const saved = localStorage.getItem('github_panel_plugins');
      if (saved) {
        const list = JSON.parse(saved);
        list.forEach(async p => {
          if (p.enabled) {
            try {
              await this.loadPlugin(p.code, p.id, false);
            } catch (e) {
              console.error(`Failed to restore plugin ${p.id}:`, e);
            }
          }
        });
      }
    } catch (e) {
      console.error('Failed to load plugins storage', e);
    }
  }
  ...
  _saveToStorage() {
    ...
    localStorage.setItem('github_panel_plugins', JSON.stringify(list));
  }

  async loadPlugin(code, id = null, save = true) {
    try {
      const func = new Function('context', 'plugin', code);
      
      const defaultId = id || 'plugin-' + Date.now() + '-' + Math.floor(Math.random() * 1000);
      const defaultPlugin = {
        id: defaultId,
        name: "Unknown Plugin",
        version: "0.0.1",
      ...
      const result = func(this.context, defaultPlugin);
      const finalPlugin = (result && typeof result === 'object') ? result : defaultPlugin;
      
      if (!finalPlugin.id) finalPlugin.id = defaultId;
      
      if (finalPlugin.style) {
        const oldStyle = document.getElementById(`plugin-style-${finalPlugin.id}`);
        if (oldStyle) oldStyle.remove();

        const style = document.createElement('style');
        style.id = `plugin-style-${finalPlugin.id}`;
        style.textContent = finalPlugin.style;
        document.head.appendChild(style);
        finalPlugin._styleEl = style;
      }

      if (finalPlugin.init) {
        await finalPlugin.init(this.context);
      }
      
      finalPlugin.code = code;
      this.plugins.set(finalPlugin.id, finalPlugin);
      
      if (save) this._saveToStorage();
      
      console.log(`Plugin loaded: ${finalPlugin.name} (${finalPlugin.id})`);
      return finalPlugin;
    } catch (e) {
      ErrorHandler.handle(e, '加载插件失败');
      throw e;
    }
  }
  ...
  unloadPlugin(id) {
    ...
    if (p.destroy) {
        try { p.destroy(this.context); } catch(e) { console.error(e); }
    }
  ...
  async importFromGitHub(url) {
    ...
    if (!url.includes('raw.githubusercontent.com')) {
      ...
      if (match) {
        ...
        if (path.endsWith('.js')) {
          ...
          targetUrl = `https://raw.githubusercontent.com/${owner}/${repo}/${branch}/${cleanPath}/plugin.js`;
        }
      }
    }
    ...
    try {
      ...
      if (!res.ok) {
        const proxyUrl = `https://mirror.ghproxy.com/${targetUrl}`;
        console.log(`Direct fetch failed, trying proxy: ${proxyUrl}`);
        const resProxy = await fetch(proxyUrl);
        if (!resProxy.ok) throw new Error(`Failed to fetch plugin: ${res.status} / ${resProxy.status}`);
        const code = await resProxy.text();
...
class GitHubPanelCore {
  ...
  loadSettings() {
    try {
      const saved = localStorage.getItem('github_panel_settings');
      if (saved) {
        const data = JSON.parse(saved);
        if (data.token) this.token = data.token;
        if (data.aiProvider) this.aiProvider = data.aiProvider;
        if (data.siliconKey) this.siliconKey = data.siliconKey;
        if (data.siliconModel) this.siliconModel = data.siliconModel;
        if (data.siliconParams) this.siliconParams = { ...this.siliconParams, ...data.siliconParams };
        if (data.customAI) this.customAI = { ...this.customAI, ...data.customAI };
        if (data.sfTranslateEnabled !== undefined) this.sfTranslateEnabled = data.sfTranslateEnabled;
        if (data.sfTranslateModel) this.sfTranslateModel = data.sfTranslateModel;
        if (data.githubModel) this.githubModel = data.githubModel;
        if (data.searchState) this.searchState = { ...this.searchState, ...data.searchState };
        this.updateAIConfig();
      }
    } catch (e) {
  ...
  saveSettings() {
    try {
      const data = {
        ...
        sfTranslateEnabled: this.sfTranslateEnabled,
        sfTranslateModel: this.sfTranslateModel,
        githubModel: this.githubModel,
        searchState: this.searchState
      };
      localStorage.setItem('github_panel_settings', JSON.stringify(data));
    } catch (e) {
...
class GitHubPanelExtension {
  ...
  getInfo() {
    return {
      ...
      name: 'GitHub 面板 Pro+',
      blocks: [
        { opcode: 'showPanel', blockType: Scratch.BlockType.COMMAND, text: '显示面板' },
        { opcode: 'hidePanel', blockType: Scratch.BlockType.COMMAND, text: '隐藏面板' },
        { opcode: 'toggleAIPopup', blockType: Scratch.BlockType.COMMAND, text: '切换 AI 悬浮窗' },
        {
          opcode: 'setToken',
  ...
  toggleAIPopup() {
    if (!this.ui.aiPopup) {
      const { popup, input, body, close } = UIComponents.createAIPopup();
      this.ui.aiPopup = popup;
      this.ui.aiPopupInput = input;
      this.ui.aiPopupBody = body;
      close.onclick = () => popup.style.display = 'none';
      document.body.appendChild(popup);
      
      let drag = { on: false, x: 0, y: 0, left: 0, top: 0 };
      popup.querySelector('div').onmousedown = (e) => {
        drag.on = true;
      ...
      window.addEventListener('mousemove', (e) => {
        ...
        popup.style.left = (drag.left + dx) + 'px';
        popup.style.top = (drag.top + dy) + 'px';
    ...
    this.ui.aiPopup.style.display = this.ui.aiPopup.style.display === 'none' ? 'flex' : 'none';
  }
  ...
  _createUI() {
    ...
    try {
      const savedPos = localStorage.getItem('github_panel_position');
      if (savedPos) {
        const { left, top } = JSON.parse(savedPos);
        this.ui.panel.style.left = left;
        this.ui.panel.style.top = top;
      }
    } catch (e) {}

    const { header, winCtrls } = UIComponents.createHeader('GitHub 面板 Pro+');
    this.ui.header = header;
    
    const minBtn = UIComponents.createWindowButton('_', { width: '28px', padding: '4px 0' });
    const fullBtn = UIComponents.createWindowButton('□', { width: '28px', padding: '4px 0' });
    const closeBtn = UIComponents.createWindowButton('×', { width: '28px', padding: '4px 0', background: 'rgba(255, 80, 80, 0.4)' });
    ...
    window.addEventListener('mouseup', () => {
      if (drag.on) {
        drag.on = false;
        localStorage.setItem('github_panel_position', JSON.stringify({
          left: this.ui.panel.style.left,
          top: this.ui.panel.style.top
        }));
      }
    });
    ...
    this.ui.tabIssuesBtn = UIComponents.createTabButton('看板');
    this.ui.tabActionsBtn = UIComponents.createTabButton('Actions');
    this.ui.tabAIBtn = UIComponents.createTabButton('AI');
    this.ui.tabCodeInputBtn = UIComponents.createTabButton('📝 代码输入');
    ...
    this.ui.tabs.appendChild(this.ui.tabIssuesBtn);
    this.ui.tabs.appendChild(this.ui.tabActionsBtn);
    this.ui.tabs.appendChild(this.ui.tabAIBtn);
    this.ui.tabs.appendChild(this.ui.tabCodeInputBtn);
    ...
    this.ui.issuesArea = this._createIssuesArea();
    this.ui.actionsArea = this._createActionsArea();
    this.ui.codeInputArea = this._createCodeInputArea();
    ...
    bodyWrap.appendChild(this.ui.issuesArea);
    bodyWrap.appendChild(this.ui.actionsArea);
    bodyWrap.appendChild(this.ui.codeInputArea);
  ...
  _createCodeInputArea() {
    ...
    const pathInput = Utils.input('文件路径 (如: src/main.js)');
    pathInput.style.flex = '1';
    pathInput.style.minWidth = '200px';

    const langSel = Utils.select([
    ...
    langSel.sel.value = 'javascript';

    const saveBtn = UIComponents.createWindowButton('💾 保存到仓库', { background: '#28a745' });
    const aiBtn = UIComponents.createWindowButton('🤖 发送到AI', { background: 'rgba(100,200,255,0.2)' });
    const clearBtn = UIComponents.createWindowButton('🗑️ 清空');
    const copyBtn = UIComponents.createWindowButton('📋 复制');
    const downloadBtn = UIComponents.createWindowButton('⬇️ 下载');

    controls.appendChild(pathInput);
    controls.appendChild(langSel.el);
    ...
    saveBtn.onclick = () => this._saveCodeInput();
    aiBtn.onclick = () => this._sendCodeToAI();
    clearBtn.onclick = () => {
    ...
    downloadBtn.onclick = () => {
      const path = pathInput.value || 'untitled.txt';
      const filename = path.split('/').pop();
      const blob = new Blob([editor.value], { type: 'text/plain' });
      Utils.download(blob, filename);
    };

    this.ui.codeInputEditor = editor;
    this.ui.codeInputPath = pathInput;
    this.ui.codeInputLang = langSel.sel;
    this.ui.codeInputStatus = status;
  ...
  _refreshPluginsList() {
    ...
    this.pluginManager.plugins.forEach((p, id) => {
      ...
      const delBtn = UIComponents.createWindowButton('卸载', { background: 'rgba(255,80,80,0.2)' });
      delBtn.onclick = () => {
  ...
  _createSearchArea() {
    ...
    repoQ.style.flex = '1';
    repoQ.value = this.core.searchState.repoQuery || '';
    ...
    userQ.style.flex = '1';
    userQ.value = this.core.searchState.userQuery || '';
    ...
    orgQ.style.flex = '1';
    orgQ.value = this.core.searchState.orgQuery || '';
  ...
  _createBrowseArea() {
    ...
    const mergeBtn = UIComponents.createWindowButton('合并分支', { background: 'rgba(100,200,100,0.2)' });
    ...
    const codeSearchInp = Utils.input('搜代码...');
    codeSearchInp.style.width = '100px';
    codeSearchInp.onkeydown = (e) => {
      if (e.key === 'Enter') this._performCodeSearch(codeSearchInp.value);
    };
    ...
    bcBranch.appendChild(codeSearchInp);
    ...
    const delFileBtn = UIComponents.createWindowButton('删除文件', { background: 'rgba(255,80,80,0.25)' });
    delFileBtn.onclick = async () => this._showDeleteFileUI();
    const graphBtn = UIComponents.createWindowButton('代码谱图', { background: 'rgba(100,200,255,0.2)' });
    graphBtn.onclick = () => this._generateCodeGraph();
    const aiReadmeBtn = UIComponents.createWindowButton('AI生成README', { background: 'rgba(200,100,255,0.2)' });
    aiReadmeBtn.onclick = () => this._generateFolderReadme();

    bc3.appendChild(membersBtn);
    bc3.appendChild(activityBtn);
    bc3.appendChild(addFolderBtn);
    bc3.appendChild(delFileBtn);
    bc3.appendChild(graphBtn);
    bc3.appendChild(aiReadmeBtn);
  ...
  _createAIArea() {
    ...
    const reviewBtn = UIComponents.createWindowButton('审阅', { display: 'none' });
  ...
  _createMyArea() {
    ...
    const mInboxBtn = UIComponents.createWindowButton('收件箱 (只读)');
    mInboxBtn.onclick = () => this._loadMyInbox();
    mControls.appendChild(mReposBtn);
    mControls.appendChild(mInboxBtn);
  ...
  _createIssuesArea() {
    const area = document.createElement('div');
    Object.assign(area.style, { display: 'none', height: '100%', flexDirection: 'column' });
    return area;
  }
  ...
  _bindEvents() {
    ...
    this.ui.tabIssuesBtn.onclick = () => this._switchMode('issues');
    this.ui.tabActionsBtn.onclick = () => this._switchMode('actions');
    this.ui.tabAIBtn.onclick = () => this._switchMode('ai');
    this.ui.tabCodeInputBtn.onclick = () => this._switchMode('codeinput');
  ...
  _switchMode(mode) {
    ...
    const isCodeInput = mode === 'codeinput';
    ...
    this.ui.issuesArea.style.display = isIssues ? 'flex' : 'none';
    this.ui.actionsArea.style.display = isActions ? 'flex' : 'none';
    this.ui.codeInputArea.style.display = isCodeInput ? 'flex' : 'none';
    ...
    this.ui.tabIssuesBtn.style.background = isIssues ? 'rgba(255,255,255,0.25)' : '';
    this.ui.tabActionsBtn.style.background = isActions ? 'rgba(255,255,255,0.25)' : '';
    this.ui.tabAIBtn.style.background = isAI ? 'rgba(255,255,255,0.25)' : '';
    this.ui.tabCodeInputBtn.style.background = isCodeInput ? 'rgba(255,255,255,0.25)' : '';
    ...
    if (isSearch) LoadingManager.setMessage('请选择方向并搜索');
      ...
      LoadingManager.setMessage('Issue/PR 看板');
      this._loadIssuesBoard();
    } else if (isActions) {
      LoadingManager.setMessage('GitHub Actions');
      this._loadActions();
    } else if (isAI) {
      ...
      LoadingManager.setMessage('代码输入模式 - 支持直接编辑和保存');
    } else if (isPlugins) {
  ...
  async _saveCodeInput() {
    const path = this.ui.codeInputPath.value.trim();
    const content = this.ui.codeInputEditor.value;
    ...
    if (!this.core.currentOwner || !this.core.currentRepo) {
      alert('请先进入一个仓库 (使用浏览模式)');
      return;
    }
    
    try {
      LoadingManager.setMessage('正在保存文件...');
      
      let sha = null;
      try {
        const existing = await this.core.apiManager.fetchJson(
          `https://api.github.com/repos/${this.core.currentOwner}/${this.core.currentRepo}/contents/${path}?ref=${this.core.currentBranch}`
        );
        sha = existing.sha;
        if (!confirm(`文件 ${path} 已存在，是否覆盖？`)) {
          LoadingManager.setMessage('保存已取消');
          return;
        }
      } catch (e) {
      }
      ...
      await this.core.apiManager.putFile(
        ...
        `Create/Update ${path} via Code Input Panel`,
        this.core.currentBranch,
        sha
      );
      
      this.ui.codeInputStatus.textContent = `✅ 文件已保存: ${path}`;
      this.ui.codeInputStatus.style.color = '#8f8';
      
      if (this.core.mode === 'browse') {
        const dirPath = path.split('/').slice(0, -1).join('/');
        if (dirPath === this.core.currentPath) {
          setTimeout(() => this.loadDir(this.core.currentPath), 1000);
        }
      }
    } catch (e) {
      ErrorHandler.handle(e, '保存代码');
      this.ui.codeInputStatus.textContent = `❌ 保存失败: ${e.message}`;
      this.ui.codeInputStatus.style.color = '#f88';
    } finally {
      setTimeout(() => {
        this.ui.codeInputStatus.style.color = '';
        this.ui.codeInputStatus.textContent = '就绪';
      }, 3000);
    }
  }
  ...
  _sendCodeToAI() {
    ...
    const lang = this.ui.codeInputLang.value;
    const path = this.ui.codeInputPath.value || '代码';
    this.ui.aiPromptInput.value = `请分析以下${lang}代码 (${path}) 并给出改进建议：\n\n${content}`;
    
    LoadingManager.setMessage('代码已发送到AI，点击"开始(流式)"进行分析');
  }
  ...
  _createRepoItem(item) {
    ...
    if (this.core.sfTranslateEnabled) {
      const transBtn = UIComponents.createWindowButton('翻译', { background: 'rgba(60,160,255,0.2)' });
      transBtn.onclick = () => {
    ...
    const introBtn = UIComponents.createWindowButton('硅基AI介绍', { background: 'rgba(255,100,200,0.2)' });
    introBtn.onclick = () => {
  ...
  async searchRepos() {
    ...
    try {
      ...
      this.core.searchState.repoQuery = qBase;
  ...
  async searchUsers() {
    ...
    try {
      ...
      this.core.searchState.userQuery = baseQ;
      this.core.saveSettings();
  ...
  async searchOrgs() {
    ...
    try {
      ...
      this.core.searchState.orgQuery = baseQ;
      this.core.saveSettings();
  ...
  async openFile(file) {
    ...
    try {
      ...
      const del = UIComponents.createWindowButton('删除此文件', { background: 'rgba(255,80,80,0.25)' });
      ...
      const editBtn = UIComponents.createWindowButton('修改模式', { background: 'rgba(255, 165, 0, 0.3)' });
      editBtn.style.display = this.core.token ? 'inline-block' : 'none';
      editBtn.onclick = () => this._toggleEditMode(file.path);
      
      const saveBtn = UIComponents.createWindowButton('保存修改 (上传)', { background: '#28a745', display: 'none' });
      ...
      const heatBtn = UIComponents.createWindowButton('贡献热力图', { background: 'rgba(200,100,255,0.2)' });
      ...
      textarea.addEventListener('contextmenu', (e) => this._handleEditorContextMenu(e));
      let touchTimer;
      textarea.addEventListener('touchstart', (e) => {
        touchTimer = setTimeout(() => this._handleEditorContextMenu(e.touches[0]), 800);
      });
  ...
  async _loadIssuesBoard() {
    LoadingManager.setMessage('Loading Issues & PRs...');
    try {
      ErrorHandler.assertRepo(this.core.currentOwner, this.core.currentRepo);
      this.ui.issuesArea.innerHTML = '';
      ...
      const openCol = createCol('Open Issues/PRs', '#28a745');
      const closedCol = createCol('Closed', '#cb2431');

      const issues = await this.core.apiManager.fetchJson(
        `https://api.github.com/repos/${this.core.currentOwner}/${this.core.currentRepo}/issues?state=all&per_page=50`
      );
      
      const openItems = issues.filter(i => i.state === 'open');
      const closedItems = issues.filter(i => i.state === 'closed');
      
      const openScroller = new VirtualScroller(openCol.list, 60);
      const closedScroller = new VirtualScroller(closedCol.list, 60);
      
      openScroller.setItems(openItems, (item) => {
        ...
        card.innerHTML = `
          <div style="font-size:12px;opacity:0.7;display:flex;justify-content:space-between">
            <span>#${item.number} ${isPR ? '🔀 PR' : '🐞 Issue'}</span>
            <span>${item.user.login}</span>
          </div>
          <div style="font-weight:500;margin:4px 0">${item.title}</div>
          <div style="font-size:11px;opacity:0.5">${item.updated_at.split('T')[0]}</div>
        `;
        card.onclick = () => window.open(item.html_url);
        return card;
      });
      
      closedScroller.setItems(closedItems, (item) => {
        const card = document.createElement('div');
        Object.assign(card.style, {
          background: 'rgba(0,0,0,0.3)',
          padding: '8px',
      ...
      board.appendChild(openCol.col);
      board.appendChild(closedCol.col);
      this.ui.issuesArea.appendChild(board);
      LoadingManager.setMessage('Board loaded.');
    } catch (e) {
      ErrorHandler.handle(e, '加载看板');
    }
  }
  ...
  async _loadActions() {
    LoadingManager.setMessage('Loading Workflows...');
    try {
      ErrorHandler.assertRepo(this.core.currentOwner, this.core.currentRepo);
      this.ui.actionsArea.innerHTML = '';
      ...
      const refreshBtn = UIComponents.createWindowButton('刷新');
      refreshBtn.onclick = () => this._loadActions();
      header.appendChild(refreshBtn);

      const workflows = await this.core.apiManager.fetchJson(
        `https://api.github.com/repos/${this.core.currentOwner}/${this.core.currentRepo}/actions/workflows`
      );
      
      if (workflows.total_count > 0) {
        const wfSel = document.createElement('select');
        Object.assign(wfSel.style, {
          background: '#333',
          color: '#fff',
          border: '1px solid #555',
          padding: '5px',
        ...
        workflows.workflows.forEach(w => {
          ...
          wfSel.appendChild(op);
        });
        const trigBtn = UIComponents.createWindowButton('手动触发 (Dispatch)', { background: '#28a745' });
        trigBtn.onclick = () => this._triggerWorkflow(wfSel.value);
        header.appendChild(wfSel);
        header.appendChild(trigBtn);
      }

      const runs = await this.core.apiManager.fetchJson(
        `https://api.github.com/repos/${this.core.currentOwner}/${this.core.currentRepo}/actions/runs?per_page=20`
      );
      
      const list = document.createElement('div');
      Object.assign(list.style, { flex: '1', overflowY: 'auto' });
      
      const scroller = new VirtualScroller(list, 70);
      scroller.setItems(runs.workflow_runs, (run) => {
        const row = document.createElement('div');
        Object.assign(row.style, Utils.itemStyle());
        const color = run.conclusion === 'success' ? '#28a745' : run.conclusion === 'failure' ? '#cb2431' : '#dbab09';
        row.innerHTML = `
          <div style="display:flex;justify-content:space-between;align-items:center">
            <div>
              <span style="color:${color};font-weight:bold">● ${run.status}</span> 
              <b>${run.name}</b> #${run.run_number}
            </div>
            <div style="font-size:11px;opacity:0.7">${run.created_at}</div>
          </div>
          <div style="font-size:12px;opacity:0.8;margin-top:4px">${run.head_commit.message}</div>
        `;
        
        const logBtn = UIComponents.createWindowButton('查看日志 (Jobs)', {
          marginTop: '5px',
          fontSize: '11px'
        });
        logBtn.onclick = () => this._showJobLogs(run.id);
      ...
      this.ui.actionsArea.appendChild(container);
      LoadingManager.setMessage('Actions loaded.');
    } catch (e) {
      ErrorHandler.handle(e, '加载Actions');
    }
  }

  async _triggerWorkflow(workflowId) {
    try {
      ErrorHandler.assertRepo(this.core.currentOwner, this.core.currentRepo);
      if (!confirm('确定要手动触发此工作流吗 (ref: main)?')) return;
      
      await this.core.apiManager.triggerWorkflow(
        this.core.currentOwner,
        this.core.currentRepo,
        workflowId,
        this.core.defaultBranch || 'main'
      );
      
      alert('触发请求已发送');
      setTimeout(() => this._loadActions(), 2000);
    } catch (e) {
      ErrorHandler.handle(e, '触发工作流');
    }
  }

  async _showJobLogs(runId) {
    try {
      ErrorHandler.assertRepo(this.core.currentOwner, this.core.currentRepo);
      const jobs = await this.core.apiManager.fetchJson(
        `https://api.github.com/repos/${this.core.currentOwner}/${this.core.currentRepo}/actions/runs/${runId}/jobs`
      );
      
      let msg = `Run ID: ${runId}\n\n`;
      jobs.jobs.forEach(j => {
        msg += `[${j.status}: ${j.conclusion}] ${j.name}\n`;
        j.steps.forEach(s => {
          msg += `  - ${s.name} (${s.conclusion})\n`;
        });
        msg += '\n';
      });
      alert(msg);
    } catch (e) {
      ErrorHandler.handle(e, '获取日志');
    }
  }
  ...
  async _generateFileHeatmap(path) {
    ...
    try {
      ...
      const close = UIComponents.createWindowButton('关闭', { marginTop: '20px', width: '100%' });
  ...
  async _showSelectiveCloneUI() {
    LoadingManager.setMessage('Loading your repos...');
    try {
      ErrorHandler.assertToken(this.core.token);
      ...
      container.innerHTML = '<h3>克隆到我的仓库 (选择性)</h3>';
      
      const repoSel = document.createElement('select');
      Object.assign(repoSel.style, {
        width: '100%',
        marginBottom: '10px',
        background: '#333',
        color: '#fff',
      ...
      repos.forEach(r => {
        const op = document.createElement('option');
        op.value = r.full_name;
        op.textContent = r.full_name;
        repoSel.appendChild(op);
      });
      
      const treeBtn = UIComponents.createWindowButton('加载当前仓库文件树', {
        width: '100%',
        marginBottom: '10px'
      });
      ...
      treeBtn.onclick = async () => {
        ...
        const tree = await this.core.apiManager.fetchJson(
          `https://api.github.com/repos/${this.core.currentOwner}/${this.core.currentRepo}/git/trees/${this.core.currentBranch}?recursive=1`
        );
      ...
      const startBtn = UIComponents.createWindowButton('开始克隆选中文件', {
        width: '100%',
        background: '#d58'
      });
      
      startBtn.onclick = async () => {
        if (selectedFiles.size === 0) return alert('未选择文件');
        const target = repoSel.value;
        const [tOwner, tRepo] = target.split('/');
        
        let count = 0;
        for (const f of selectedFiles) {
          try {
            const blob = await this.core.apiManager.fetchJson(f.url);
            await this.core.apiManager.putFile(tOwner, tRepo, f.path, blob.content, `Clone ${f.path}`, this.core.defaultBranch, null, true);
            count++;
            LoadingManager.setMessage(`Cloned ${count}/${selectedFiles.size}`);
          } catch (e) {
            console.error(e);
          }
        }
        alert(`完成。成功克隆 ${count} 个文件。`);
      };
      
      container.appendChild(repoSel);
      container.appendChild(treeBtn);
      container.appendChild(treeContainer);
      container.appendChild(startBtn);
      this.ui.mainArea.appendChild(container);
      LoadingManager.setMessage('请选择目标仓库');
    } catch (e) {
      ErrorHandler.handle(e, '加载仓库列表');
    }
  }
  ...
  _renderActionRow() {
    ...
    const btnClone = UIComponents.createWindowButton('从我的仓库克隆', {
      background: 'rgba(60,160,255,0.2)'
    });
    btnClone.onclick = () => this._showCloneUI();
    
    const btnCloneToMine = UIComponents.createWindowButton('克隆到我的仓库', {
      background: 'rgba(255,100,200,0.2)'
    });
    btnCloneToMine.onclick = () => this._showSelectiveCloneUI();

    this.ui.actionRow.appendChild(btnNew);
    this.ui.actionRow.appendChild(btnUp);
    this.ui.actionRow.appendChild(btnClone);
    this.ui.actionRow.appendChild(btnCloneToMine);
  }
  ...
  _showAIRewritePanel() {
    ...
    const closeBtn = UIComponents.createWindowButton('×', { padding: '2px 8px' });
    ...
    const copyAllBtn = UIComponents.createWindowButton('复制完整代码', { flex: '1' });
    copyAllBtn.onclick = () => {
      Utils.copyToClipboard(this.ui.editorTextarea.value);
    };

    const dlBtn = UIComponents.createWindowButton('下载仓库(Zip)', { flex: '1' });
    dlBtn.onclick = () => {
  ...
  async _performCodeSearch(keyword) {
    try {
      ErrorHandler.assert(keyword, '请输入搜索关键词');
      ErrorHandler.assertRepo(this.core.currentOwner, this.core.currentRepo);
      
      LoadingManager.setMessage(`Searching code: ${keyword}...`);
      this.ui.mainArea.innerHTML = '';
      
      const q = `${keyword} repo:${this.core.currentOwner}/${this.core.currentRepo}`;
      const data = await this.core.apiManager.fetchJson(
      ...
      scroller.setItems(data.items, (item) => {
        const row = document.createElement('div');
        Object.assign(row.style, Utils.itemStyle());
        row.innerHTML = `<div><b>${item.name}</b> <span style="opacity:0.6">${item.path}</span></div>`;
        row.style.cursor = 'pointer';
        row.onclick = () => this.openFile(item);
        return row;
      });
      
      const back = UIComponents.createWindowButton('返回文件列表');
      back.onclick = () => this._goBack();
      this.ui.mainArea.insertBefore(back, this.ui.mainArea.firstChild);
      
      LoadingManager.setMessage('Search done.');
    } catch (e) {
      ErrorHandler.handle(e, '搜索代码');
    }
  }

  _generateCodeGraph() {
    this._switchMode('ai');
    this.ui.aiPromptInput.value = `Please analyze the file structure and dependencies of this repository (based on cached context or general knowledge). Generate a Mermaid JS "classDiagram" or "graph TD" code block representing the relationship between main modules/files. Only output the Mermaid code.`;
    if (this.core.cacheManager.repoTextCache.size === 0) {
      alert('请先使用 "读取整个仓库到缓存" 功能，以便 AI 分析代码关系。');
    }
  }
  ...
  async _showCloneUI() {
    LoadingManager.setMessage('正在获取您的仓库列表...');
    try {
      ErrorHandler.assertToken(this.core.token);
      ...
      const title = document.createElement('h3');
      title.textContent = '从您的仓库克隆 -> 到当前位置';
      ...
      const targetPathInp = Utils.input('目标路径 (当前文件夹)');
      targetPathInp.value = this.core.currentPath || '';
      targetPathInp.style.width = '100%';
      targetPathInp.style.marginBottom = '10px';
      
      const startBtn = UIComponents.createWindowButton('开始克隆', {
        width: '100%',
        padding: '10px',
        background: '#2a8'
      });
      ...
      startBtn.onclick = async () => {
        const sourceFullName = repoSel.value;
        const targetBase = targetPathInp.value;
        const confirmMsg = `确定要从 ${sourceFullName} 克隆所有文件到当前仓库的 ${targetBase || '根目录'} 吗？`;
        if (confirm(confirmMsg)) {
          this.core._cloneAbort = false;
          await this._performClone(sourceFullName, this.core.currentOwner, this.core.currentRepo, targetBase);
        }
      };
      ...
      container.appendChild(repoSel);
      container.appendChild(targetPathInp);
      container.appendChild(startBtn);
      container.appendChild(backBtn);
      this.ui.mainArea.appendChild(container);
      LoadingManager.setMessage('请选择源仓库');
    } catch (e) {
      ErrorHandler.handle(e, '获取仓库列表');
    }
  }

  async _showCloneToMineUI() {
    LoadingManager.setMessage('正在获取您的仓库列表...');
    try {
      ErrorHandler.assertToken(this.core.token);
      ErrorHandler.assertRepo(this.core.currentOwner, this.core.currentRepo);
      ...
      const title = document.createElement('h3');
      title.textContent = `克隆此仓库 (${this.core.currentOwner}/${this.core.currentRepo}) -> 到您的仓库`;
      ...
      const targetPathInp = Utils.input('目标路径 (可选)');
      targetPathInp.placeholder = '例如: lib/cloned-code';
      targetPathInp.style.width = '100%';
      targetPathInp.style.marginBottom = '10px';
      ...
      startBtn.onclick = async () => {
        const targetFullName = repoSel.value;
        const [targetOwner, targetRepo] = targetFullName.split('/');
        const targetBase = targetPathInp.value;
        const sourceFullName = `${this.core.currentOwner}/${this.core.currentRepo}`;
        const confirmMsg = `确定要将 ${sourceFullName} 克隆到您的仓库 ${targetFullName} 的 ${targetBase || '根目录'} 吗？`;
        if (confirm(confirmMsg)) {
          this.core._cloneAbort = false;
          await this._performClone(sourceFullName, targetOwner, targetRepo, targetBase);
        }
      };
  ...
  async _performClone(sourceFullName, targetOwner, targetRepo, targetBase) {
    LoadingManager.setMessage(`正在获取 ${sourceFullName} 的文件列表...`);
    try {
      const [srcOwner, srcRepo] = sourceFullName.split('/');
      const repoMeta = await this.core.apiManager.fetchJson(
        `https://api.github.com/repos/${srcOwner}/${srcRepo}`
      );
      const branch = repoMeta.default_branch;
      
      const treeData = await this.core.apiManager.fetchJson(
        `https://api.github.com/repos/${srcOwner}/${srcRepo}/git/trees/${branch}?recursive=1`
      );
      
      if (treeData.truncated) {
        if (!confirm('源仓库文件过多，列表被截断。克隆可能不完整。继续吗？')) return;
      }
      ...
      LoadingManager.setMessage(`开始克隆 ${files.length} 个文件...`);
      
      for (const f of files) {
        if (this.core._cloneAbort) throw new Error('用户中止');
        
        try {
          const blobData = await this.core.apiManager.fetchJson(f.url);
          const targetPath = targetBase ? (targetBase.replace(/\/$/, '') + '/' + f.path) : f.path;
          ...
          await this.core.apiManager.putFile(
            ...
            targetPath,
            blobData.content,
            `Clone ${f.path} from ${sourceFullName}`,
            this.core.defaultBranch || 'main',
            null,
            true
          );
          ...
          count++;
          LoadingManager.setMessage(`克隆进度: ${count}/${files.length}`);
        } catch (e) {
          fails++;
          console.error('Clone file error:', f.path, e);
        }
      }
      
      alert(`克隆完成。\n成功: ${count}\n失败: ${fails}`);
      if (targetOwner === this.core.currentOwner && targetRepo === this.core.currentRepo) {
        this.loadDir(this.core.currentPath);
      }
    } catch (e) {
      ErrorHandler.handle(e, '克隆仓库');
    }
  }

  async _generateFolderReadme() {
    LoadingManager.setMessage('正在分析文件夹内容...');
    try {
      ErrorHandler.assertToken(this.core.token);
      ErrorHandler.assertRepo(this.core.currentOwner, this.core.currentRepo);
      
      const items = await this.core.apiManager.fetchJson(
        `https://api.github.com/repos/${this.core.currentOwner}/${this.core.currentRepo}/contents/${this.core.currentPath}?ref=${this.core.currentBranch}`
      );
      
      const fileNames = items.map(i => i.name).join(', ');
      const prompt = `I have a folder in a GitHub repo (${this.core.currentRepo}). Path: ${this.core.currentPath || 'root'}. Files in this folder: ${fileNames}. Please generate a concise README.md content for this folder in Markdown format. Explain what this folder likely contains based on file names. Use Simplified Chinese.`;
      ...
      const confirmSave = confirm('AI 生成完毕。预览:\n\n' + content.slice(0, 200) + '...\n\n是否保存为 README.md?');
      if (confirmSave) {
        const path = this.core.currentPath ? `${this.core.currentPath}/README.md` : 'README.md';
        await this.core.apiManager.putFile(
          this.core.currentOwner,
          this.core.currentRepo,
          path,
      ...
      ErrorHandler.handle(e, '生成README');
    }
  }
  ...
  async _loadMyRepos() {
    ...
    try {
      ...
      const createBtn = UIComponents.createWindowButton('+ 新建仓库', { background: '#28a745' });
  ...
  async _loadMyInbox() {
    LoadingManager.setMessage('Loading inbox...');
    try {
      ErrorHandler.assertToken(this.core.token);
      this.ui.mainArea.innerHTML = '';
      
      const notifs = await this.core.apiManager.fetchJson('https://api.github.com/notifications');
      
      if (notifs.length === 0) {
        this.ui.mainArea.innerHTML = '<div style="padding:20px;opacity:0.7">没有新通知 (Inbox empty)</div>';
        return;
      }
      
      const scroller = new VirtualScroller(this.ui.mainArea, 60);
      scroller.setItems(notifs, (n) => {
        const row = document.createElement('div');
        Object.assign(row.style, Utils.itemStyle());
        
        const sub = document.createElement('div');
        sub.style.fontWeight = 'bold';
        sub.textContent = n.subject.title;
        
        const meta = document.createElement('div');
        meta.style.fontSize = '12px';
        meta.style.opacity = '0.8';
        meta.textContent = `${n.repository.full_name} [${n.subject.type}] (${n.reason})`;
      ...
      LoadingManager.setMessage('Inbox loaded.');
    } catch (e) {
      ErrorHandler.handle(e, '加载收件箱');
    }
  }
  ...
  async _showProfile(login, isOrg) {
    ...
    try {
      ...
      const backBtn = UIComponents.createWindowButton('返回', { marginLeft: 'auto' });
      backBtn.onclick = () => {
        this.ui.mainArea.innerHTML = '';
      ...
      const viewReposBtn = UIComponents.createWindowButton('查看该用户/组织的仓库', { width: '100%' });
