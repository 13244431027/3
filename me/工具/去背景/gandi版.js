(function (Scratch) {
  ...
  function isVM(o) {
    return !!(o && o.runtime && typeof o.runtime.getTargetForStage === 'function');
  }

  function findVM() {
    const cands = [];
    if (Scratch) {
      cands.push(Scratch.vm);
      if (Scratch.vm && Scratch.vm.vm) cands.push(Scratch.vm.vm);
      if (Scratch.vm && Scratch.vm.runtime) cands.push(Scratch.vm.runtime.vm);
    }
    if (typeof window !== 'undefined') {
      cands.push(window.vm);
      cands.push(window.Gandi && window.Gandi.vm);
      cands.push(window.__gandi__ && window.__gandi__.vm);
      cands.push(window.ScratchVM);
    }
    for (let i = 0; i < cands.length; i++) {
      if (isVM(cands[i])) return cands[i];
    }
    const maybeRuntime =
      (Scratch && Scratch.vm && Scratch.vm.storage && Scratch.vm) ||
      (Scratch && Scratch.runtime) ||
      null;
    if (maybeRuntime && typeof maybeRuntime.getTargetForStage === 'function') {
      return { runtime: maybeRuntime, __shim: true };
    }
    return null;
  }
  ...
  const BlockType = (Scratch && Scratch.BlockType) || {
    COMMAND: 'command',
    REPORTER: 'reporter',
    BOOLEAN: 'Boolean'
  };
  const ArgumentType = (Scratch && Scratch.ArgumentType) || { STRING: 'string' };

  function castString(v) {
    if (Scratch && Scratch.Cast && Scratch.Cast.toString) return Scratch.Cast.toString(v);
    return v === null || v === undefined ? '' : String(v);
  }

  const unsandboxed = !!(Scratch && Scratch.extensions && Scratch.extensions.unsandboxed);
  ...
  const state = {
    ...
    modelProgress: 0,
    busy: false,
    statusText: unsandboxed || vm ? '等待上传图片' : '当前环境受限，功能可能不可用',
    sourceURL: '',
    resultURL: '',
    fileName: 'image',
    lastError: ''
  ...
  function loadLib() {
    ...
    state.libPromise = Promise.resolve()
      .then(() => dynamicImport(CDN + 'browser.mjs'))
      .then((mod) => {
        state.lib = mod;
  ...
  function runModel(file) {
    return loadLib().then((lib) => {
      const removeBackground = lib.removeBackground || (lib.default && lib.default.removeBackground);
      if (typeof removeBackground !== 'function') {
        throw new Error('模型运行库缺少 removeBackground 方法');
      }
  ...
  // 不依赖 vm.addCostume：优先用官方 API，缺失时直接走 target.addCostume + renderer
  function attachCostume(target, md5ext, costume) {
    ...
    // 手动补齐 costume 的渲染字段，等价于 loadCostume 的结果
    return prepareCostumeForRender(costume).then(() => {
  ...
  // 为 costume 生成 renderer 的 skinId 和 asset 引用
  ...
  function saveResultToTarget(target, name) {
    ...
    return loadImage(state.resultURL).then((img) => {
      ...
      return attachCostume(target, md5ext, costume).then(() => {
  ...
  class BackgroundRemoverPanel {
    ...
    toCostume(args, util) {
      const target = (util && util.target) || getEditingTarget();
