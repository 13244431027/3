...
(async function (Scratch) {
  ...
  if (!Scratch.extensions.unsandboxed) {
    throw new Error(`"更多字段"扩展必须在非沙盒模式下运行。`);
  }
  ...
  let Blockly = null; // 使用Blockly因为它比ScratchBlocks更容易
  ...
  function tryUseScratchBlocks(_sb) {
    ...
    SVGTextElement.prototype.setAttribute = function(attr, val, ...args) {
      if (String(val) === 'NaN' && (attr === 'x' || attr === 'y') && this.getAttribute('class') === 'blocklyText') {
        ...
        _setAttribute.call(this, nattr, `尝试在此文本节点上进行非法设置。${attr.toUpperCase()}被设置为NaN。`);
        return _setAttribute.call(this, attr, '0', ...args);
      }
      return _setAttribute.call(this, attr, val, ...args);
    };

    // 修复 size_.height 的bug
    ...
    implementations.FieldInlineSlider = class FieldInlineSlider extends Blockly.FieldNumber {
      ...
      showEditor_() {
        ...
        div.append(document.createTextNode('最小值: '));
        div.append(minInput);
        div.append(document.createElement('br'));
        div.append(document.createTextNode('值: '));
        div.append(valInput);
        div.append(document.createElement('br'));
        div.append(document.createTextNode('最大值: '));
        div.append(maxInput);
    ...
    // 图标来自: https://fonts.google.com/icons
    ...
    implementations.FieldFileInput = class FieldFileInput extends Blockly.Field {
      ...
      _onUploadClick(e) {
        ...
        fileInput.addEventListener('change', (event) => {
          ...
          let noFileErr = () => false;
          const unfe = () => {
            if (fileList.length < 0) noFileErr = () => fiErr(true, '没有上传文件？');
            return true;
          };
      ...
      showEditor_(forceShow) {
        ...
        if (!!this._getFileData().at(0)) {
          ...
          clearBtn.textContent = '清除文件';
          div.appendChild(document.createElement('br'));
        }
        div.appendChild(document.createTextNode('上传为: '));
        div.appendChild(outputOptions);
        outputOptions.innerHTML = outputOptions_temp.optValues;
        div.appendChild(document.createElement('br'));
        div.appendChild(document.createTextNode('限制文件类型: '));
  ...
  // 如果Scratch.gui是一个对象，则将"Blockly"传递给tryUseScratchBlocks
        ...
        blocks: [
          {
            ...
            text: '文件 [FILE] Snap布尔值 [BOOL] 滑块 [NUM]',
            arguments: {
              FILE: {
